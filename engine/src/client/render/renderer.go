...
func (r *Renderer) SetMetricsEnabled(enabled bool) {
	if r != nil {
...
func (r *Renderer) Metrics() Metrics {
	...
	return Metrics{UploadTime: r.uploadTime, SurfaceRecreations: r.surfaceRecreations}
}
...
// ScreenSize returns the current drawable display size. Callers use this as
// the final presentation target so the upload surface does not follow the
// logical game buffer or the native texture-density intermediate.
func (r *Renderer) ScreenSize() image.Point {
	if r == nil || r.sc == nil {
		return image.Point{}
	}
	return r.sc.ScreenSize()
}

func (r *Renderer) present(img *noximage.Image16) {
	var uploadStart time.Time
	if r.metricsEnabled && img != nil {
	...
	if img != nil {
		...
		if bsz := r.backbuf.Size(); sz != bsz || r.filtering != r.backbufFilt {
			...
			r.backbuf = r.sc.NewSurface(sz, r.filtering)
			r.backbufFilt = r.filtering
			r.surfaceRecreations++
		}
		r.backbuf.Update(r.buf)
		if !uploadStart.IsZero() {
			r.uploadTime += time.Since(uploadStart)
		}
