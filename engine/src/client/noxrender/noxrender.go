...
func NewRender(log *slog.Logger, f Framer) *NoxRender {
	hdPerf := newTextureDensityPerf()
	r := &NoxRender{
		f:                             f,
		hdPerf:                        hdPerf,
		textureDensityEnabled:         true,
		textureDensityConfiguredScale: configuredTextureDensityScale(),
	}
	r.initText()
...
type NoxRender struct {
	...
	textureDensity        []textureDensityDraw
	textureDensityBuffers []textureDensityBuffers
	textureDensityBytes   int
	textureDensityScale   int
	textureDensityEnabled bool
	// textureDensityConfiguredScale is supplied by the launcher. A zero value
	// keeps the unit-test/direct-renderer behavior that discovers scale per frame.
	textureDensityConfiguredScale int
	// texturePresentationScale is stable for the current menu/game mode. Zero
	// means the legacy per-frame discovery path is being used.
	texturePresentationScale   int
	textureOwners              []uint32
	textureOwnerSeq            uint32
	texturePresentation        *noximage.Image16
	textureDisplayPresentation *noximage.Image16
	textureReplayWritten       []bool
	// The completed native-density frame can be retained across otherwise
	// unchanged logical frames. The companion logical copy and replay keys let
	// the presentation pass update only changed 16x16 logical tiles.
	texturePresentationBase       []uint16
	texturePresentationOwners     []uint32
	texturePresentationBaseRect   image.Rectangle
	texturePresentationBaseStride int
	texturePresentationKeys       []textureDensityReplayKey
	texturePresentationDirty      []bool
	texturePresentationValid      bool
...
func (r *NoxRender) ClearScreen(cl color.Color) {
	if r.pix != nil {
		r.markTexturePrimitiveRect(r.pix.Rect)
	}
