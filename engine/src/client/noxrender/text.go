...
func (r *RenderFonts) Load(lang int) error {
	...
	for i := range noxFontFiles {
		...
		f.Font = fnt
		// Native font sprites are optional. Their cells are exactly 2x the
		// logical font cells, and are used only by the 2x presentation replay.
		// Absence deliberately preserves the stock rendering path.
		f.Native, err = loadNativeFont(datapath.Data(fname))
		if err != nil {
			return fmt.Errorf("cannot load native font: %w", err)
		}
...
// NativeFont returns the matching 2x bitmap face, when its optional sprite
// file is installed. The logical face remains the layout authority.
func (r *RenderFonts) NativeFont(face font.Face) font.Face {
	if face == nil {
		return nil
	}
	for i := range noxFontFiles {
...
func loadNativeFont(path string) (font.Face, error) {
	f, err := ifs.Open(path + ".hd2" + noxfont.Ext)
	if os.IsNotExist(err) {
		return nil, nil
	}
	if err != nil {
...
func (r *NoxRender) DrawStringStyle(font font.Face, str string, pos image.Point) int { // nox_xxx_drawStringStyle_43F7B0
	...
	return r.withLogicalText(func() int {
		r.text.advance = 1
...
func (r *NoxRender) textBounds(fnt font.Face, s string, pos image.Point) image.Rectangle {
	s = strings.NewReplacer("\n", "", "\r", "").Replace(s)
	w := r.advanceString(fnt, s, image.Point{})
	h := fnt.Metrics().Height.Round()
	if h < 1 {
		h = fnt.Metrics().CapHeight.Round()
	}
...
func (r *NoxRender) drawString(fnt font.Face, s string, pos image.Point) int {
	...
	bounds := r.textBounds(fnt, s, pos)
	native, _, useNative := r.textureDensityTextEligible(fnt)
	// drawString is the smallest safe operation boundary for text. Preflight
	// every rune before emitting any HD record so a missing native glyph keeps
	// the whole operation on the logical path (including '?' replacement).
	if useNative && !textureDensityTextSupported(native, s) {
		useNative = false
		if r.hdPerf != nil {
			r.hdPerf.fontFallbacks++
			r.hdPerf.reject(textureDensityRejectMissingGlyph)
		}
	} else if r.textureDensityEnabled && r.texturePresentationScale == 2 && native == nil && r.hdPerf != nil {
		r.hdPerf.fontFallbacks++
	}
	var background []uint16
	if useNative {
		background = r.captureTextureBackgroundRect(bounds)
		useNative = len(background) != 0
	}
	if !useNative {
		r.markTexturePrimitiveRect(bounds)
	}
	...
	r.text.DrawString(s)
	if useNative && !r.queueTextureText(fnt, native, s, pos, bounds, background, r.text.advance) {
		// A bounded queue can decline a pathological text-heavy frame. Preserve
		// the safe logical fallback instead of letting an older sprite leak over it.
		r.markTexturePrimitiveRect(bounds)
	}
...
// withLogicalText keeps compound operations (wrapping, highlight, and bold
// style) atomic. Their constituent drawString calls have no safe way to
// reserve the complete replay queue, so the logical renderer is authoritative
// for the entire operation.
func (r *NoxRender) withLogicalText(fn func() int) int {
	enabled := r.textureDensityEnabled
	if enabled {
		r.textureDensityEnabled = false
		if r.texturePresentationScale == 2 && r.hdPerf != nil {
			r.hdPerf.fontFallbacks++
			r.hdPerf.reject(textureDensityRejectCompound)
		}
	}
	defer func() { r.textureDensityEnabled = enabled }()
	return fn()
}
