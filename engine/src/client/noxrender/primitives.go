...
func (r *NoxRender) DrawPixel(pos image.Point, cl color.Color) {
	...
	r.markTexturePrimitiveRect(image.Rectangle{Min: pos, Max: pos.Add(image.Pt(1, 1))})
...
func (r *NoxRender) DrawLineHorizontal(x1, y, x2 int, cl color.Color) {
	...
	r.markTexturePrimitiveRect(image.Rect(xmin, y, xmax+1, y+1))
	pix := r.PixBuffer()
	cl16 := noxcolor.ToRGBA5551Color(cl)
...
func (r *NoxRender) drawLineVertical(x, y1, y2 int, cl color.Color) {
	...
	r.markTexturePrimitiveRect(image.Rect(x, ymin, x+1, ymax+1))
	pix := r.PixBuffer()
	cl16 := noxcolor.ToRGBA5551Color(cl)
	for y := ymin; y <= ymax; y++ {
		pix.SetRGBA5551(x, y, cl16)
...
func (r *NoxRender) DrawLine(p1, p2 image.Point, cl color.Color) {
	...
	r.markTexturePrimitiveRect(image.Rect(minInt(p1.X, p2.X), minInt(p1.Y, p2.Y), maxInt(p1.X, p2.X)+1, maxInt(p1.Y, p2.Y)+1))
	if p1.X == p2.X {
...
func (r *NoxRender) DrawRectFilledOpaque(x, y, w, h int, cl color.Color) {
	...
	r.markTexturePrimitiveRect(image.Rect(rx, ry, rx+rw, ry+rh))
	sz := r.PixBuffer().Rect
...
func (r *NoxRender) DrawRectFilledAlpha(x, y, w, h int) {
	...
	d := r.Data()
	if !d.Clip() {
		r.markTexturePrimitiveRect(image.Rect(x, y, x+w, y+h))
		r.drawRectFilledAlpha(x, y, w, h)
		return
	}
	rc := image.Rect(x, y, x+w, y+h)
	if rc := rc.Intersect(d.ClipRect()); !rc.Empty() {
		r.markTexturePrimitiveRect(rc)
...
func (r *NoxRender) DrawCircleOpaque(cx, cy, rad int, cl color.Color) {
	r.markTexturePrimitiveRect(image.Rect(cx-rad, cy-rad, cx+rad+1, cy+rad+1))
	d := r.Data()
	pix := r.PixBuffer()
	cl16 := noxcolor.ToRGBA5551Color(cl)
