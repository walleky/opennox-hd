...
func (r *NoxRender) DrawImage16(img Image16, pos image.Point) {
	...
	if p, ok := img.(*Image); ok && p != nil {
		_ = p.Pixdata()
		marked = r.textureDensityEligible(p)
		if marked && r.textureDensityNeedsBackground(p) {
			background = r.captureTextureBackground(p, pos)
		}
	}
	...
	r.markTextureOwnership(img, pos, marked || len(r.textureDensity) != 0)
	if p, ok := img.(*Image); ok && p != nil && marked {
		r.queueTextureDensity(p, pos, background)
	}
}
