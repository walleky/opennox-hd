...
func (c *Client) copyPixBuffer() {
	// Keep the native-density image at its composed size. The renderer uploads
	// it once and lets OpenGL scale it into the drawable viewport; resizing it
	// here uses a software bilinear pass every frame and can starve input.
	c.Win.CopyBuffer(c.r.CopyPresentationBuffer())
...
func (c *Client) gameResetVideoMode(inMenu, force bool) error {
	if c.r != nil {
		c.r.SetTextureDensityMode(inMenu)
	}
