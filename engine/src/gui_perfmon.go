...
func (c *Client) DrawPerfmon(m *Perfmon) {
	if !m.enabled {
		if c.r != nil {
			c.r.SetTextureDensityMetricsEnabled(false)
		}
		if c.Win != nil {
			c.Win.SetMetricsEnabled(false)
		}
		if c.Inp != nil {
			c.Inp.SetMetricsEnabled(false)
		}
		m.hdTraceArmed = false
		return
	}
	if c.r != nil {
		c.r.SetTextureDensityMetricsEnabled(true)
	}
	if c.Win != nil {
		c.Win.SetMetricsEnabled(true)
	}
	if c.Inp != nil {
		c.Inp.SetMetricsEnabled(true)
	}
	if !m.hdTraceArmed {
		c.r.ArmTextureDensityTrace()
		m.hdTraceArmed = true
	}
	m.LogHD(c)
